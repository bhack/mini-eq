...
class MiniEqWindowLayoutMixin:
    def build_window_content(self, auto_route: bool) -> None:
        ...
        output_label = Gtk.Label(label="EQ output", xalign=0.0)
        ...
        self.analyzer_switch.connect("state-set", self.on_analyzer_changed)
        self.analyzer_freeze_switch.connect("state-set", self.on_analyzer_freeze_changed)
        ...
        self.bypass_switch.connect("state-set", self.on_bypass_changed)
        self.route_switch.connect("state-set", self.on_route_changed)
